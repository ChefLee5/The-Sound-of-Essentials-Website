...
14. **Digital Product Policy (Zero Return Policy / No Money-Back Guarantees)** — All products sold across the platform (Rhythm Ready Workbook PDF, Essential Picture Dictionary EPUB, Album downloads, and Digital Bundles) are instant downloadable digital goods. In accordance with standard market digital goods policy, do NOT offer, mention, or promise money-back guarantees, 30-day refund policies, or returns anywhere on the site, sales pages, checkout funnels, email sequences, or ad blueprints. Customer assurance must be anchored in product quality, instant digital delivery, printable/tablet-ready compatibility, and lifetime family access.
15. **Trilingual Learning Architecture (English · Español · Français):** SOE features a triple-language acoustic immersion curriculum designed for ages 2–7 without digital screens. It integrates English, Spanish, and French phonemic pathways into the core music catalog and tactile print materials:
   - **Interactive Language Switcher:** `web/src/components/Navbar.jsx` provides an interactive language selector with flags (`🇺🇸 EN`, `🇪🇸 ES`, `🇫🇷 FR`) for desktop (including outside-click detection) and a segmented pill controller for the mobile navigation drawer.
   - **Homepage Showcase:** `web/src/pages/Home.jsx` features Section 5.5 ("The Trilingual Acoustic Pathway"), spotlighting the English pathway (Track 04, phonemic foundations), Spanish pathway (Track 11, rhythmic counting and call-and-response), and French pathway (Track 08 *Le Cheval*, natural prosody and diction), linked to the 4,000+ Word Picture Dictionary.
